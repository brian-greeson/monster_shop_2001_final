...
RSpec.describe 'As an merchant user', type: :feature do
  ...
  describe 'I can manage my discounts' do
    it 'via a link from my dashboard' do
      ...
      within ".item-#{@tire.id}-discounts" do
        ...
        expect(page).to have_content("Discount of: #{discount2.discount}% on quantities higher than #{discount2.quantity} units")
        expect(page).to have_content("Discount of: #{discount3.discount}% on quantities higher than #{discount3.quantity} units")
      end
    end    
  end
